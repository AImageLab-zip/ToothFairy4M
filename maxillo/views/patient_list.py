...
from ..models import (
    Patient as MaxilloPatient,
    Folder as MaxilloFolder,
    Tag as MaxilloTag,
)
from .domain import get_namespace
...
def home(request):
    ...
    if request.user.is_authenticated:
        ...
        if request.user.is_staff:
            ...
            projects = all_projects.filter(id__in=accessible_project_ids).order_by(
                "name"
            )
...
def patient_list(request):
    ...
    has_files_relation = any(
        rel.name == "files" for rel in Patient._meta.related_objects
    )
    ...
    if _Job is not None and any(
        rel.name == "jobs" for rel in Patient._meta.related_objects
    ):
    ...
    try:
        if current_project_id:
            proj = Project.objects.prefetch_related("modalities").get(
    ...
    # Filter patients by the project's active modalities when the Patient model
    # has no direct project FK (e.g. laparoscopy project → only 'video' modality).
    if allowed_modalities and not any(
        field.name == "project" for field in Patient._meta.fields
    ):
        slugs = [m.slug for m in allowed_modalities if getattr(m, "slug", "")]
        if slugs:
            patients = patients.filter(modalities__slug__in=slugs).distinct()
    ...
    for patient in patients_to_process:
        ...
        manual_classification = next(
            (c for c in classifications if c.classifier == "manual"), None
        )
        ai_classification = next(
            (c for c in classifications if c.classifier == "pipeline"), None
        )
        ...
        voice_caption_processed = voice_captions and all(
        ...
        for m in allowed_modalities:
            ...
            status = "absent"
            has_any_files = (
                slug in files_by_modality and len(files_by_modality[slug]) > 0
            )
        ...
        patient_data = {
            ...
            "has_ai_only": ai_classification is not None
            and manual_classification is None,
            "needs_processing": manual_classification is None
            ...
            "modality_statuses": {
                ms["slug"]: ms["status"] for ms in modality_status_list
            },
    ...
    for folder in folders:
        ...
        folders_with_counts.append({"folder": folder, "patient_count": folder_count})
    ...
    if has_status_filters:
        ...
        for item in patients_with_status:
            modality_status_by_slug = {
                ms["slug"]: ms["status"] for ms in item.get("modality_status_list", [])
            }
            ...
            if "voice" in status_filters:
                ...
                if any(
                    job.modality_slug == "voice" and job.status == "failed"
                    for job in patient_jobs
                ):
                    voice_status = "failed"
