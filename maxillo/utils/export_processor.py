...
class ExportProcessor:
    ...
    def query_patients(self):
        ...
        patients = Patient.objects.filter(folder_id__in=self.folder_ids) if self.folder_ids else Patient.objects.none()
    ...
    def collect_files(self, patients):
        ...
        for patient in patients:
            ...
            for file_reg in patient_files:
                ...
                if is_mapped_file_type or is_processed_fallback:
                    ...
                    logger.debug(f"    Modality slug: {modality_slug}, file_path: {file_reg.file_path}, exists: {os.path.exists(file_reg.file_path) if file_reg.file_path else False}")
                    
                    # Special handling for CBCT processed: files are stored in metadata['files']
                    if file_reg.file_type == 'cbct_processed' and file_reg.metadata and 'files' in file_reg.metadata:
                        ...
                        for file_type_key, file_data in cbct_files.items():
                            if isinstance(file_data, dict) and 'path' in file_data:
                                ...
                                if os.path.exists(file_path):
                                    ...
                                    logger.warning(f"CBCT processed file not found: {file_path}")
                    elif file_reg.file_path and os.path.exists(file_reg.file_path):
                        # Standard single-file export
                        logger.info(f"  Adding file: {file_reg.file_type} (modality: {modality_slug}) from {file_reg.file_path}")
                        files_to_export.append({
                            'type': 'file',
                            'patient': patient,
                            'file_registry': file_reg,
                        ...
                        total_size += file_reg.file_size
                    elif not file_reg.file_path:
                        logger.warning(f"FileRegistry {file_reg.id} ({file_reg.file_type}) has no file_path and no metadata files")
                    else:
        ...
        logger.info(f"Total files collected: {len(files_to_export)}, total size: {total_size} bytes")
                        ...
                        if modality_slug == 'cbct' and 'cbct_file_type' in file_info:
                            ...
                            if cbct_file_type.startswith('structures_mesh'):
                                if cbct_file_type == 'structures_mesh':
                                    ...
                                    suffix = cbct_file_type.replace('structures_mesh_', '')
                                    filename = f'structures_{suffix}.stl' if suffix else 'structures.stl'
                            else:
                                filename = cbct_filename_map.get(cbct_file_type, os.path.basename(source_path))
                        else:
    ...
    def process_export(self):
        ...
        try:
            ...
            # Create export directory in /dataset/exports
            dataset_path = getattr(settings, 'DATASET_PATH', '/dataset')
            export_dir = Path(dataset_path) / 'exports'
            try:
                os.makedirs(export_dir, exist_ok=True)
            except (PermissionError, OSError) as e:
                raise Exception(f"Cannot create export directory {export_dir}: {e}")
            ...
            self.export.mark_completed(
                file_path=str(export_path),
                file_size=actual_size
            )
            
            logger.info(f"Export {self.export.id} completed successfully. Size: {actual_size} bytes")
            
        except Exception as e:
            logger.error(f"Error processing export {self.export.id}: {e}", exc_info=True)
