...
export function applyShapesMixin(proto) {
        proto._registerShape = function (type, konvaNode, options) {
            ...
            var shape = {
                ...
                frameTime: (typeof options.frameTime === 'number') ? options.frameTime : this._currentVideoTime(),
            };

            if (shape.konvaNode && typeof shape.konvaNode.visible === 'function') {
                shape.konvaNode.visible(Math.abs(this._currentVideoTime() - shape.frameTime) <= FRAME_TOLERANCE);
            }
        ...
        proto._renderShapesList = function () {
            var self = this;
            ...
            if (this._filterShapesCurrentFrame) {
                var currentTime = this._currentVideoTime();
        ...
        proto._bindPolygonShape = function (shape) {
            var self = this;
        ...
        proto._segmentDistanceSq = function (ax, ay, bx, by, px, py) {
        ...
        proto._insertVertexIntoShape = function (shape, pos) {
        ...
        proto._removeVertexFromShape = function (shape, vi) {
            if (!shape || shape.type !== 'polygon') return;
            var points = shape.konvaNode.points().slice();
        ...
        proto._deleteShape = function (id) {
            ...
            var layer = (s.konvaNode && typeof s.konvaNode.getLayer === 'function')
                ? s.konvaNode.getLayer()
                : null;
            ...
            if (layer && typeof layer.draw === 'function') {
                layer.draw();
            } else if (region && region.layer && typeof region.layer.draw === 'function') {
                region.layer.draw();
            } else if (this.stage && typeof this.stage.draw === 'function') {
                this.stage.draw();
            }
            ...
            if (typeof this._onShapeDeleted === 'function') {
                this._onShapeDeleted(s);
            }
        ...
        proto._deleteLastShape = function () {
            if (!this.shapes.length) return;
            this._deleteShape(this.shapes[this.shapes.length - 1].id);
        };

        proto._selectShape = function (id) {
            var self = this;
        ...
        proto._jumpToShapeFrame = function (shapeId) {
        ...
        proto._changeShapeRegion = function (shapeId, newRegionId) {
            var shape = this.shapes.find(function (s) { return s.id === shapeId; });
            if (!shape) return;
        ...
        proto._showRegionSelector = function (shapeId, clickEvent) {
            var self = this;
            var shape = this.shapes.find(function (s) { return s.id === shapeId; });
        ...
        proto._updateShapeVisibility = function () {
        ...
        proto._enterAnnotationMode = function () {
            ...
            if (typeof this._updateSubsampledPreviewMode === 'function') {
                this._updateSubsampledPreviewMode();
            }
        };

        proto._exitAnnotationMode = function () {
            if (typeof this._syncMainVideoToDisplayedTime === 'function') {
            ...
            if (typeof this._clearMaskOverlay === 'function') {
        ...
        proto._bindToolbar = function () {
            ...
            _on('toggle-regions-visibility-btn', 'click', function () {
                self._toggleAllRegionsVisibility();
            });
        ...
        proto._startDrawing = function (pos) {
        ...
        proto._continueDrawing = function (pos) {
        ...
        proto._polyAddVertex = function (pos) {
            var region = this._activeRegion();
            if (!region) return;
        ...
        proto._polyUpdateGuide = function (pos) {
        ...
        proto._resetPolyState = function () {
            this._polyPoints = [];
            this._polyLine   = null;
